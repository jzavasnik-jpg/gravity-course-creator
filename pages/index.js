...
Example format: ["They can build a profitable business 3x faster than traditional methods", "They get results in 90 days or less with our proven system"]`;
  ...
  // Call the Vercel API endpoint (which will call Gemini safely)
  ...
  const generateAvatars = async () => {
    ...
    try {
      ...
      setApiError(`Failed to generate avatars: ${error.message}`);
    } finally {
  ...
  // Update avatar data
  const updateAvatar = async (gender, field, value) => {
  ...
  return (
    <div className="h-full flex flex-col space-y-6 overflow-y-auto">
      ...
      <p className="text-slate-600">
        Here are your ideal customer profiles with detailed emotional pain points mapped to the Six S's framework.
      </p>
              ...
              <table className="w-full border-collapse border border-slate-300">
                ...
                <tbody>
                  {Object.entries(avatars.male.sixSsPainPoints || {}).map(([feeling, painPoint]) => (
                    <tr key={feeling} className="hover:bg-slate-25">
